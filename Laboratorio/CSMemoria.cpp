...
void CSMemoria::mostrar() {
	...
	for (int i = 0; i < MAX; i++) {
		int rel = edir - lexical_cast<string>(i).length();
		string dir = " " + string(rel, '0') + lexical_cast<string>(i) + " ";
		rel = edat - lexical_cast<string>(mem[i].dato).length();
		string dat = string(rel, ' ') + lexical_cast<string>(mem[i].dato);
		rel = eid - mem[i].id.length();
		string id = string(rel, ' ') + mem[i].id;
		rel = edir - lexical_cast<string>(mem[i].link).length();
		string lnk = "  " + string(rel, ' ') + lexical_cast<string>(mem[i].link) + " ";
		out += "|" + dir + "|" + dat + "|" + id + "|" + lnk + "|\n";
	}
	out += lin;
	out += "Libre = " + lexical_cast<string>(libre) + "\n";
	out += "Espacios libres = " + lexical_cast<string>(espacio_disponible()) + "\n";
	out += "Espacios ocupados = " + lexical_cast<string>(espacio_ocupado()) + "\n";
	cout << out;
}